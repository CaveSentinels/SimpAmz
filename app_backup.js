...
function db_insert_products(records) {
...
app.get('/admin/load_data', function(req, res) {
    var success_msg_base = "Product data import succeeded.";
    var failure_msg_base = "Product data import failed.";

    var record = {
        Id : "", ASIN : "", Description : "", Categories: "", Title : "", Group : ""
    };
    var records = [];
    ...
    lineReader.eachLine(data_file, function(line, last) {   // func_01
        if (line.indexOf("Id:") >= 0) {
            if (state == 1 /*Not in any record*/) {
                ...
                record.Categories = categories.join();
                categories = [];
                records.push([record.Id, record.ASIN, record.Description, record.Categories, record.Title, record.Group]);
        ...
        if (line.indexOf("categories:") >= 0 || line.indexOf("reviews:") > 0 || state == 3/*In categories*/) {
            ...
            if (line.indexOf("reviews:") >= 0) {
                ...
                categories.push(subStr);
            }
        }

        if (last || (records.length > 0 && records.length % 100 == 0)) {
            var insert_count = 0;
            var insert_errors = 0;
            ...
            pool.query(sql_stmt, [records], function(err, result) { // func_02
                if (err) {
                    if (last) {
                        return res.status(500).json(ret_value(
                            failure_msg_base,
                        ...
                        records = [];   // Clear all the records so far.
                    }
                } else {
                    insert_count = result.affectedRows;
                    insert_errors = records.length - insert_count;

                    console.log("==============================");
                    console.log("Data import completed:");
                    console.log("Total: " + insert_count + " product(s)");
                    console.log("Error: " + insert_errors + " product(s)");
                    console.log("==============================");
                    ...
                    for (var i = 0; i < records.length; ++i) {
                        inventories.push([records[i][0], default_quantity]);
                    }
                    sql_stmt = "INSERT INTO `Inventory` (`ProdID`, `Quantity`) VALUES ?";
                    pool.query(sql_stmt, [inventories], function(err, result) { // func_03
                        if (err) {
                            if (last) {
                                return res.status(500).json(ret_value(
                                    failure_msg_base,
                            ...
                            console.log("==============================");
                            console.log("Data import completed:");
                            console.log("Total: " + insert_count + " inventories");
                            console.log("Error: " + insert_errors + " inventories");
                            console.log("==============================");
